...
/// Weekly session planner: assign existing routines to days of the week.
...
class _WorkoutPlanningScreenState extends State<WorkoutPlanningScreen> {
  ...
  Future<void> _loadData() async {
    ...
    await DatabaseHelper.instance.ensureRecurringSessionsForWeek(_weekStart);
    final routines = await DatabaseHelper.instance.getRoutines();
    final scheduled =
        await DatabaseHelper.instance.getScheduledSessionsForWeek(_weekStart);
    final recurring = await DatabaseHelper.instance.getRecurringSchedules();
    if (!mounted) return;
    setState(() {
      _routines = routines;
  ...
  Future<void> _planDay(DateTime day) async {
    ...
    if (result.clearExisting) {
      if (existing != null) {
        await DatabaseHelper.instance.deleteWorkoutSession(existing.id);
      }
      await DatabaseHelper.instance.deleteRecurringScheduleForWeekday(day.weekday);
      _loadData();
      return;
    }
    ...
    if (result.repeatWeekly) {
      await DatabaseHelper.instance.upsertRecurringSchedule(
        RecurringSchedule(
  ...
  Color _goalColor(RoutineGoal goal) {
    switch (goal) {
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final goalColor = matchedRoutine != null
                          ? _goalColor(matchedRoutine.goal)
                          : null;
                      ...
                      return _DayPlanCard(
                        ...
                        goalColor: session != null &&
                                session.status != WorkoutStatus.skipped
                            ? goalColor
                            : null,
                        onTap: () => _planDay(day),
                        onRemove: session != null
...
class _RoutinePickerSheetState extends State<_RoutinePickerSheet> {
  ...
  void _selectRoutine(Routine routine) {
    Navigator.pop(
      context,
      _PlanPickerResult(routine: routine, repeatWeekly: _repeatWeekly),
    );
  }
          ...
          Padding(
            ...
            child: Material(
              ...
              child: SwitchListTile(
                ...
                subtitle: const Text(
                  'Automatically schedule this routine on the same weekday',
                  style: TextStyle(
                    color: AppColors.textSecondary,
                    fontSize: 12,
                  ),
